...
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.ContentDispositionUtils.attachment;
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.DocumentController.PATH_DELETE;
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.DocumentController.PATH_DOWNLOAD;
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.DocumentController.PATH_FIND_ALL;
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.DocumentController.PATH_FIND_ONE;
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.DocumentController.PATH_UPLOAD;
import static com.github.ingogriebsch.sample.spring.data.jpa.postgresql.lob.handling.DocumentController.REQUEST_PART_SOURCE;
...
public class DocumentControllerTest {
    ...
    @Test
    public void findAll_should_return_matching_page() throws Exception {
        Pageable pageable = new PageRequest(1, 5);
        Page<Document> page = new PageImpl<>(newArrayList(), pageable, 100);
        given(documentRepository.findAll(pageable)).willReturn(page);

        ResultActions actions =
            mockMvc.perform(get(PATH_FIND_ALL + "?page={page}&size={size}", pageable.getPageNumber(), pageable.getPageSize())
                .accept(APPLICATION_JSON_UTF8));
        actions.andExpect(status().isOk());

        actions.andExpect(jsonPath("$.size").value(page.getSize()));
        actions.andExpect(jsonPath("$.number").value(page.getNumber()));
        actions.andExpect(jsonPath("$.totalElements").value(page.getTotalElements()));
